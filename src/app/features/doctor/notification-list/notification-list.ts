...
export class NotificationList implements OnInit, OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      if (wrapper && wrapper.timestamp > this.lastNotificationTimestamp) {
        ...
        const exists = this.notifyFacade.notifications().some((n) => n.id === newNotification.id);

        if (!exists) {
          this.notifyFacade.notifications.update((current) => [newNotification, ...current]);
  ...
  markAllAsRead() {
    ...
    this.notifyFacade.markAllAsRead(command).subscribe({
      next: () => {
        this.notifyFacade.notifications.update((list) => list.map((n) => ({ ...n, isRead: true })));
